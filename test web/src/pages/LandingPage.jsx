...
import { products, farmers, categories, images } from '../data/mockData';
              ...
              return (
                <Link to={`/product/${p.id}`} key={p.id} className={`product-card fade-in-up stagger-${i + 1}`} id={`product-card-${p.id}`}>
                  ...
                  <div className="product-card-body">
                    ...
                    <button className="add-cart-btn" onClick={e => { e.preventDefault(); addToCart(p); }} id={`add-cart-${p.id}`}>{t('addToCart')}</button>
                  </div>
                </Link>
              );
            })}
          ...
          <div className="farmers-grid">
            {farmers.slice(0, 4).map((f, i) => (
              <div key={f.id} className={`farmer-card fade-in-up stagger-${i + 1}`} id={`farmer-${f.id}`}>
                <div className="farmer-avatar">{f.image ? <img src={f.image} alt={f.name} /> : '👨‍🌾'}</div>
                <h3>{lang === 'en' ? f.name : f.nameHi}</h3>
                <div className="location"><span>📍</span>{lang === 'en' ? f.location : f.locationHi}</div>
